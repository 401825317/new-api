...
func TestQuotaExactBoundsAreNotClamped(t *testing.T) {
	...
	quota, err := QuotaRoundStrict(float64(MaxQuota))
...
func TestQuotaClampAuditMapSerializesNonFiniteOriginal(t *testing.T) {
	tests := []struct {
		...
		{name: "nan", original: math.NaN(), want: "NaN"},
		{name: "positive infinity", original: math.Inf(1), want: "+Inf"},
		{name: "negative infinity", original: math.Inf(-1), want: "-Inf"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.want, audit["original"])
			assert.NotEmpty(t, MapToJsonStr(map[string]interface{}{"quota_saturation": audit}))
		})
	}
}
